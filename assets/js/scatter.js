...
$(() => {
    ...
    function searchClick() {
        let source = $("#idSource").val();
        const showSD = $("#idShowSD").is(':checked');

        if (source === "" ) {
            source = "S01";
        }

        const target = `/scatter/data/${source}`;
        console.log(target, showSD);

        $.getJSON(target, function(data) {
            if (data.status !== undefined && data.status === 10) {
                layer.alert("未找到指定数据");
                return;
            }
            $("#searchForm").hide();
            renderChart({chart: gChart, data, showSD});
        });
    }
    searchClick();
    ...
    // render chart
    function renderChart({ chart, data, showSD} ) {
        if (chart !== undefined) {
            chart.clear();
        }
